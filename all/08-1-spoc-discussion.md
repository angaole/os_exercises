...
> 实现信号量的方案:<br />
对于每个进程设立一个信号处理列表。这里记录了每个信号标识和运行程序地址的映射关系。<br />
每当运行一个signal(SIGINT, signal_callback_handler);函数时，会进行系统调用，在系统调用的内核处理函数中会将SIGINT对应的程序调用地址设置为signal_callback_handler，这里修改的是current进程的信号处理列表。 <br />
当系统收到信号时，会调用中断处理例程，在中断处理例程中会判断这是否是一个信号，如果是则跳转到信号处理的内核函数。在该内核函数中，操作系统内核会查找current进程的信号处理列表，找到其程序处理地址，然后执行一个函数调用，在用户运行栈上增加处理程序的栈帧，然后修改PC为该地址，转而运行该处理程序，处理程序结束后，会根据函数调用的退出方式，继续回到程序原地址继续执行。<br />
